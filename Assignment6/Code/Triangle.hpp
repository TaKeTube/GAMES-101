...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    // Vector3f S1 = crossProduct(ray.direction, E2), S2 = crossProduct(S, E1);
    // float inv = 1/dotProduct(S1, E1);
    // u = inv * dotProduct(S1, S);
    // v = inv * dotProduct(S2, ray.direction);
    // t_tmp = inv * dotProduct(S2, E2);
    // if(u>=0 && v>=0 && (1-u-v)>=0 && t_tmp>0){
    //     inter.happened = true;
    //     inter.coords = ray.origin + t_tmp * ray.direction;
    //     inter.normal = normal;
    //     inter.distance = t_tmp;
